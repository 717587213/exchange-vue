...
!function(b){"function"==typeof define&&define.amd?define(["jquery"],b):"object"==typeof exports?module.exports=b:b(jQuery)}(function(c){function n(v){var u=v||window.event,s=k.call(arguments,1),w=0,g=0,f=0,q=0,o=0,i=0;if(v=c.event.fix(u),v.type="mousewheel","detail"in u&&(f=-1*u.detail),"wheelDelta"in u&&(f=u.wheelDelta),"wheelDeltaY"in u&&(f=u.wheelDeltaY),"wheelDeltaX"in u&&(g=-1*u.wheelDeltaX),"axis"in u&&u.axis===u.HORIZONTAL_AXIS&&(g=-1*f,f=0),w=0===f?g:f,"deltaY"in u&&(f=-1*u.deltaY,w=f),"deltaX"in u&&(g=u.deltaX,0===f&&(w=-1*g)),0!==f||0!==g){if(1===u.deltaMode){var t=c.data(this,"mousewheel-line-height");w*=t,f*=t,g*=t}else if(2===u.deltaMode){var r=c.data(this,"mousewheel-page-height");w*=r,f*=r,g*=r}if(q=Math.max(Math.abs(f),Math.abs(g)),(!b||q<b)&&(b=q,a(u,q)&&(b/=40)),a(u,q)&&(w/=40,g/=40,f/=40),w=Math[w>=1?"floor":"ceil"](w/b),g=Math[g>=1?"floor":"ceil"](g/b),f=Math[f>=1?"floor":"ceil"](f/b),e.settings.normalizeOffset&&this.getBoundingClientRect){var p=this.getBoundingClientRect();o=v.clientX-p.left,i=v.clientY-p.top}return v.deltaX=g,v.deltaY=f,v.deltaFactor=b,v.offsetX=o,v.offsetY=i,v.deltaMode=0,s.unshift(v,w,g,f),l&&clearTimeout(l),l=setTimeout(j,200),(c.event.dispatch||c.event.handle).apply(this,s)}}function j(){b=null}function a(g,f){return e.settings.adjustOldDeltas&&"mousewheel"===g.type&&f%120==0}var l,b,h=["wheel","mousewheel","DOMMouseScroll","MozMousePixelScroll"],m="onwheel"in document||document.documentMode>=9?["wheel"]:["mousewheel","DomMouseScroll","MozMousePixelScroll"],k=Array.prototype.slice;if(c.event.fixHooks)for(var d=h.length;d;)c.event.fixHooks[h[--d]]=c.event.mouseHooks;var e=c.event.special.mousewheel={version:"3.1.12",setup:function(){if(this.addEventListener)for(var f=m.length;f;)this.addEventListener(m[--f],n,!1);else this.onmousewheel=n;c.data(this,"mousewheel-line-height",e.getLineHeight(this)),c.data(this,"mousewheel-page-height",e.getPageHeight(this))},teardown:function(){if(this.removeEventListener)for(var f=m.length;f;)this.removeEventListener(m[--f],n,!1);else this.onmousewheel=null;c.removeData(this,"mousewheel-line-height"),c.removeData(this,"mousewheel-page-height")},getLineHeight:function(g){var f=c(g),i=f["offsetParent"in c.fn?"offsetParent":"parent"]();return i.length||(i=c("body")),parseInt(i.css("fontSize"),10)||parseInt(f.css("fontSize"),10)||16},getPageHeight:function(f){return c(f).height()},settings:{adjustOldDeltas:!0,normalizeOffset:!0}};c.fn.extend({mousewheel:function(f){return f?this.bind("mousewheel",f):this.trigger("mousewheel")},unmousewheel:function(f){return this.unbind("mousewheel",f)}})});
var main,setData,switchPeriod,_Global;
window.KLINEBOJ = {}
window._w_ = {}
window._w_.kiline = {
  times: '30min',
...
mEves.prototype.addHandler = function(o, f) {
    if (this._indexOf(o, f) < 0) this._handlers.push({
        obj:o,
        func:f
    });
};
...
AssignExpr.prototype.assign = function(index) {
    ...
    if (eEnv.get()._firstIndex >= 0) if (isNaN(this._buf[index]) && !isNaN(this._buf[index - 1])) throw this._name + ".assign(" + index + "): NaN";
};
...
RangeExpr.prototype.reserve = function(rid, count) {
    if (this._rid < rid) {
        for (var c = count; c > 0; c--) this._buf.push({
            resultA:NaN,
            result:NaN
        });
    }
...
StdExpr.prototype.calcResult = function(index, resultA) {
    ...
    if (index > first) {
        ...
        if (n >= index + 1 - first) {
            n = index + 1 - first;
            stdData.resultMA = this._stdBuf[index - 1].resultMA * (1 - 1 / n) + resultA / n;
        } else {
            stdData.resultMA = this._stdBuf[index - 1].resultMA + (resultA - this._buf[index - n].resultA) / n;
        }
        var sum = 0;
        for (var i = index - n + 1; i <= index; i++) sum += Math.pow(this._buf[i].resultA - stdData.resultMA, 2);
...
StdExpr.prototype.reserve = function(rid, count) {
    if (this._rid < rid) {
        for (var c = count; c > 0; c--) this._stdBuf.push({
            resultMA:NaN
        });
    }
...
hlcInd.prototype._cTruct = function() {
    ...
    this._add_outP(new OutputExpr("CLOSE", new CloseExpr(), _out_put_style.Line, Theme.Co._indtor0));
    this._add_outP(new _range_put("MA", new MaExpr(new CloseExpr(), M1), _out_put_style.Line, Theme.Co._indtor1));
};
...
_vo_indtor.prototype._cTruct = function() {
    ...
    this._add_pmeter(M1);
    this._add_pmeter(M2);
    var VOLUME = new OutputExpr("VOLUME", new VolumeExpr(), _out_put_style.VolumeStick, Theme.Co.Text4);
    this._add_outP(VOLUME);
    this._add_outP(new _range_put("MA", new MaExpr(VOLUME, M1), _out_put_style.Line, Theme.Co._indtor0));
    this._add_outP(new _range_put("MA", new MaExpr(VOLUME, M2), _out_put_style.Line, Theme.Co._indtor1));
};
...
_MACD_indtor.prototype._cTruct = function() {
    ...
    var DIF = new OutputExpr("DIF", new SubExpr(new EmaExpr(new CloseExpr(), SHORT), new EmaExpr(new CloseExpr(), LONG)));
    this._add_outP(DIF);
    var DEA = new OutputExpr("DEA", new EmaExpr(DIF, MID));
    this._add_outP(DEA);
    var MACD = new OutputExpr("MACD", new MulExpr(new SubExpr(DIF, DEA), new ConstExpr(2)), _out_put_style.MACDStick);
    this._add_outP(MACD);
};
...
_DMI_indtor.prototype._cTruct = function() {
    ...
    this._add_pmeter(N);
    this._add_pmeter(MM);
    var MTR = new AssignExpr("MTR", new ExzmExpr(new MaxExpr(new MaxExpr(new SubExpr(new HighExpr(), new LowExpr()), new AbsExpr(new SubExpr(new HighExpr(), new RefExpr(new CloseExpr(), new ConstExpr(1))))), new AbsExpr(new SubExpr(new RefExpr(new CloseExpr(), new ConstExpr(1)), new LowExpr()))), N));
    this.addAssign(MTR);
    var HD = new AssignExpr("HD", new SubExpr(new HighExpr(), new RefExpr(new HighExpr(), new ConstExpr(1))));
    this.addAssign(HD);
    var LD = new AssignExpr("LD", new SubExpr(new RefExpr(new LowExpr(), new ConstExpr(1)), new LowExpr()));
    this.addAssign(LD);
    var DMP = new AssignExpr("DMP", new ExzmExpr(new IfExpr(new AndExpr(new GtExpr(HD, new ConstExpr(0)), new GtExpr(HD, LD)), HD, new ConstExpr(0)), N));
    this.addAssign(DMP);
    var DMM = new AssignExpr("DMM", new ExzmExpr(new IfExpr(new AndExpr(new GtExpr(LD, new ConstExpr(0)), new GtExpr(LD, HD)), LD, new ConstExpr(0)), N));
    this.addAssign(DMM);
    var PDI = new OutputExpr("PDI", new MulExpr(new DivExpr(DMP, MTR), new ConstExpr(100)));
    this._add_outP(PDI);
    var MDI = new OutputExpr("MDI", new MulExpr(new DivExpr(DMM, MTR), new ConstExpr(100)));
    this._add_outP(MDI);
    var ADX = new OutputExpr("ADX", new ExzmExpr(new MulExpr(new DivExpr(new AbsExpr(new SubExpr(MDI, PDI)), new AddExpr(MDI, PDI)), new ConstExpr(100)), MM));
    this._add_outP(ADX);
...
D__indtor.prototype._cTruct = function() {
    ...
    this._add_pmeter(N2);
    this._add_pmeter(M);
    var DIF = new OutputExpr("DIF", new SubExpr(new MaExpr(new CloseExpr(), N1), new MaExpr(new CloseExpr(), N2)));
    this._add_outP(DIF);
...
TRIX_indtor.prototype._cTruct = function() {
    ...
    this._add_pmeter(N);
    this._add_pmeter(M);
    var MTR = new AssignExpr("MTR", new EmaExpr(new EmaExpr(new EmaExpr(new CloseExpr(), N), N), N));
    this.addAssign(MTR);
    var TRIX = new OutputExpr("TRIX", new MulExpr(new DivExpr(new SubExpr(MTR, new RefExpr(MTR, new ConstExpr(1))), new RefExpr(MTR, new ConstExpr(1))), new ConstExpr(100)));
    this._add_outP(TRIX);
...
BRAR_indtor.prototype._cTruct = function() {
    ...
    var BR = new OutputExpr("BR", new MulExpr(new DivExpr(new SumExpr(new MaxExpr(new ConstExpr(0), new SubExpr(new HighExpr(), REF_CLOSE_1)), N), new SumExpr(new MaxExpr(new ConstExpr(0), new SubExpr(REF_CLOSE_1, new LowExpr())), N)), new ConstExpr(100)));
    this._add_outP(BR);
    var AR = new OutputExpr("AR", new MulExpr(new DivExpr(new SumExpr(new SubExpr(new HighExpr(), new OpenExpr()), N), new SumExpr(new SubExpr(new OpenExpr(), new LowExpr()), N)), new ConstExpr(100)));
    this._add_outP(AR);
};
...
VR_indtor.prototype._cTruct = function() {
    ...
    var REF_CLOSE_1 = new AssignExpr("REF_CLOSE_1", new RefExpr(new CloseExpr(), new ConstExpr(1)));
    this.addAssign(REF_CLOSE_1);
    var TH = new AssignExpr("TH", new SumExpr(new IfExpr(new GtExpr(new CloseExpr(), REF_CLOSE_1), new VolumeExpr(), new ConstExpr(0)), N));
    this.addAssign(TH);
    var TL = new AssignExpr("TL", new SumExpr(new IfExpr(new LtExpr(new CloseExpr(), REF_CLOSE_1), new VolumeExpr(), new ConstExpr(0)), N));
    this.addAssign(TL);
    var TQ = new AssignExpr("TQ", new SumExpr(new IfExpr(new EqExpr(new CloseExpr(), REF_CLOSE_1), new VolumeExpr(), new ConstExpr(0)), N));
    this.addAssign(TQ);
    var VR = new OutputExpr("VR", new MulExpr(new DivExpr(new AddExpr(new MulExpr(TH, new ConstExpr(2)), TQ), new AddExpr(new MulExpr(TL, new ConstExpr(2)), TQ)), new ConstExpr(100)));
    this._add_outP(VR);
...
OBV_indtor.prototype._cTruct = function() {
    ...
    var REF_CLOSE_1 = new AssignExpr("REF_CLOSE_1", new RefExpr(new CloseExpr(), new ConstExpr(1)));
    this.addAssign(REF_CLOSE_1);
    var VA = new AssignExpr("VA", new IfExpr(new GtExpr(new CloseExpr(), REF_CLOSE_1), new VolumeExpr(), new NegExpr(new VolumeExpr())));
    this.addAssign(VA);
    var OBV = new OutputExpr("OBV", new SumExpr(new IfExpr(new EqExpr(new CloseExpr(), REF_CLOSE_1), new ConstExpr(0), VA), new ConstExpr(0)));
    this._add_outP(OBV);
...
_EMV_indtor.prototype._cTruct = function() {
    ...
    this._add_pmeter(N);
    this._add_pmeter(M);
    var VOLUME = new AssignExpr("VOLUME", new DivExpr(new MaExpr(new VolumeExpr(), N), new VolumeExpr()));
    this.addAssign(VOLUME);
    var MID = new AssignExpr("MID", new MulExpr(new DivExpr(new SubExpr(new AddExpr(new HighExpr(), new LowExpr()), new RefExpr(new AddExpr(new HighExpr(), new LowExpr()), new ConstExpr(1))), new AddExpr(new HighExpr(), new LowExpr())), new ConstExpr(100)));
    this.addAssign(MID);
    var EMV = new OutputExpr("EMV", new MaExpr(new DivExpr(new MulExpr(MID, new MulExpr(VOLUME, new SubExpr(new HighExpr(), new LowExpr()))), new MaExpr(new SubExpr(new HighExpr(), new LowExpr()), N)), N));
    this._add_outP(EMV);
...
RSI_indtor.prototype._cTruct = function() {
    ...
    this._add_outP(new OutputExpr("RSI1", new MulExpr(new DivExpr(new SmaExpr(new MaxExpr(CLOSE_LC, new ConstExpr(0)), N1, new ConstExpr(1)), new SmaExpr(new AbsExpr(CLOSE_LC), N1, new ConstExpr(1))), new ConstExpr(100))));
    this._add_outP(new OutputExpr("RSI2", new MulExpr(new DivExpr(new SmaExpr(new MaxExpr(CLOSE_LC, new ConstExpr(0)), N2, new ConstExpr(1)), new SmaExpr(new AbsExpr(CLOSE_LC), N2, new ConstExpr(1))), new ConstExpr(100))));
    this._add_outP(new OutputExpr("RSI3", new MulExpr(new DivExpr(new SmaExpr(new MaxExpr(CLOSE_LC, new ConstExpr(0)), N3, new ConstExpr(1)), new SmaExpr(new AbsExpr(CLOSE_LC), N3, new ConstExpr(1))), new ConstExpr(100))));
};
...
WR_indtor.prototype._cTruct = function() {
    ...
    this.addAssign(LLV1);
    var WR1 = new OutputExpr("WR1", new MulExpr(new DivExpr(new SubExpr(HHV, new CloseExpr()), new SubExpr(HHV, LLV)), new ConstExpr(100)));
    this._add_outP(WR1);
    var WR2 = new OutputExpr("WR2", new MulExpr(new DivExpr(new SubExpr(HHV1, new CloseExpr()), new SubExpr(HHV1, LLV1)), new ConstExpr(100)));
    this._add_outP(WR2);
};
...
SAR_indtor.prototype._cTruct = function() {
    ...
    this._add_outP(new OutputExpr("SAR", new SarExpr(N, MIN, STEP, MAX), _out_put_style.SARPoint));
};
...
KDJ_indtor.prototype._cTruct = function() {
    ...
    var LLV = new AssignExpr("LLV", new LlvExpr(new LowExpr(), N));
    this.addAssign(LLV);
    var RSV = new AssignExpr("RSV", new MulExpr(new DivExpr(new SubExpr(new CloseExpr(), LLV), new SubExpr(HHV, LLV)), new ConstExpr(100)));
    this.addAssign(RSV);
    var K = new OutputExpr("K", new SmaExpr(RSV, M1, new ConstExpr(1)));
    this._add_outP(K);
    var D = new OutputExpr("D", new SmaExpr(K, M2, new ConstExpr(1)));
    this._add_outP(D);
    var J = new OutputExpr("J", new SubExpr(new MulExpr(K, new ConstExpr(3)), new MulExpr(D, new ConstExpr(2))));
    this._add_outP(J);
};
...
ROC_indtor.prototype._cTruct = function() {
    ...
    this._add_pmeter(N);
    this._add_pmeter(M);
    var REF_CLOSE_N = new AssignExpr("REF_CLOSE_N", new RefExpr(new CloseExpr(), N));
    this.addAssign(REF_CLOSE_N);
    var ROC = new OutputExpr("ROC", new MulExpr(new DivExpr(new SubExpr(new CloseExpr(), REF_CLOSE_N), REF_CLOSE_N), new ConstExpr(100)));
    this._add_outP(ROC);
...
MTM_indtor.prototype._cTruct = function() {
    ...
    this._add_pmeter(N);
    this._add_pmeter(M);
    var MTM = new OutputExpr("MTM", new SubExpr(new CloseExpr(), new RefExpr(new CloseExpr(), N)));
    this._add_outP(MTM);
...
PSY_indtor.prototype._cTruct = function() {
    ...
    this._add_pmeter(N);
    this._add_pmeter(M);
    var PSY = new OutputExpr("PSY", new MulExpr(new DivExpr(new CountExpr(new GtExpr(new CloseExpr(), new RefExpr(new CloseExpr(), new ConstExpr(1))), N), N), new ConstExpr(100)));
    this._add_outP(PSY);
...
STOCHRSI_indtor.prototype._cTruct = function() {
    ...
    var CLOSE_LC = new AssignExpr("CLOSE_LC", new SubExpr(new CloseExpr(), LC));
    this.addAssign(CLOSE_LC);
    var RSI = new AssignExpr("RSI", new MulExpr(new DivExpr(new SmaExpr(new MaxExpr(CLOSE_LC, new ConstExpr(0)), N, new ConstExpr(1)), new SmaExpr(new AbsExpr(CLOSE_LC), N, new ConstExpr(1))), new ConstExpr(100)));
    this.addAssign(RSI);
    var STOCHRSI = new OutputExpr("STOCHRSI", new MulExpr(new DivExpr(new MaExpr(new SubExpr(RSI, new LlvExpr(RSI, M)), P1), new MaExpr(new SubExpr(new HhvExpr(RSI, M), new LlvExpr(RSI, M)), P1)), new ConstExpr(100)));
...
var Pushing = {
    ...
    Response:function(marketFrom, type, coinVol, data) {
        ...
        if (Pushing.marketFrom != marketFrom || Pushing.type != type || Pushing.coinVol != coinVol) return;
        _Global._KData = data;
    ...
    Start:function(callback) {
        ...
        _chart_man.getInstance().getChart().updateDataAndDisplay2();
    },
    Stop:function() {
...
Chart.prototype.updateDataAndDisplay2 = function() {
    ...
    _Global.limit = "1000";
    this.setTitle();
    _chart_man.getInstance().setCurrentDataSource("frame0.k0", "行情图" + "." + this._market_from + "." + this._time + "." + this._money_type + "." + this._contract_unit);
    _chart_man.getInstance().setNormalMode();
    var f = _Global.chartMgr.getDataSource("frame0.k0").getLastDate();
    if (f == -1) {
        _Global.rParam = setHttpRequestParam(_Global.mark_from, _Global.time_type, "1000", null);
        RequestData(true);
    } else {
        _Global.rParam = setHttpRequestParam(_Global.mark_from, _Global.time_type, null, f.toString());
        RequestData();
    }
...
Chart.prototype.setCurrentFuture = function(content) {
    ...
    window.KLINEBOJ._current_future_change.raise(_alias, _type);
    this.updateDataAndDisplay();
};
...
_chart_area.prototype.layout = function(left, top, right, bottom, forceChange) {
    left <<= 0;
    if (this._left != left) {
        this._left = left;
        this._changed = true;
...
_indtorArea.prototype.onMouseMove = function(x, y) {
    var mgr = _chart_man.getInstance();
    if (mgr._capturingMouseArea == this) if (this._dragStarted == false) if (this._oldX != x || this._oldY != y) this._dragStarted = true;
    if (this._dragStarted) {
        mgr.hideCursor();
        mgr.getTimeline(this.getDataSourceName()).move(x - this._oldX);
        return this;
...
_chart_area_group.prototype.getHighlightedArea = function() {
    if (this._highlightedArea != null) return this._highlightedArea.getHighlightedArea();
    return null;
};
...
TableLayout.prototype.measure = function(context, width, height) {
    ...
    if (timeline.getFirstIndex() >= 0) {
        var firstIndexes = [];
        for (rw = minRW; rw < maxRW; rw += nw) firstIndexes.push(timeline.calcFirstIndex(timeline.calcColumnCount(width - rw)));
...
TableLayout.prototype.layout = function(left, top, right, bottom, forceChange) {
    TableLayout.__classId.layout.call(this, left, top, right, bottom, forceChange);
    ...
    if (!mgr.getCaptureMouseWheelDirectly()) {
        for (i = 0, cnt += 2; i < cnt; i += 2) {
            if (this._areas[i].isSelected()) {
                ...
                context.strokeRect(this.getLeft() + .5, this.getTop() + .5, this.getWidth() - 1, this.getHeight() - 1);
                break;
            }
        }
    }
...
_dockable_layout.prototype.layout = function(left, top, right, bottom, forceChange) {
    _dockable_layout.__classId.layout.call(this, left, top, right, bottom, forceChange);
    left = this.getLeft();
    top = this.getTop();
    right = this.getRight();
...
_chart_man.prototype.bindCanvas = function(layer, canvas) {
    if (layer == "main") {
        ...
        if (this._captureMouseWheelDirectly) $(this._overlayCanvas).bind("mousewheel", mouseWheel);
    }
};
...
_chart_man.prototype.setCaptureMouseWheelDirectly = function(v) {
    ...
    if (v) $(this._overlayCanvas).bind("mousewheel", mouseWheel); else $(this._overlayCanvas).unbind("mousewheel");
};
...
_chart_man.prototype.init = function() {
    ...
    DefaultTemplate.loadTemplate("frame0.k0", "行情图", "frame0.order", "0.order", "frame0.trade", "0.trade");
...
_chart_man.prototype.drawMain = function(frameName, context) {
    ...
    if (!drawn) {
        for (var it in this._areas) {
            if (this._areas[it].getFrameName() == frameName && !iITance(this._areas[it], _chart_area_group)) this.drawAreaMain(context, this._areas[it]);
        }
    }
    var e;
...
_chart_man.prototype.drawOverlay = function(frameName, context) {
    for (var n in this._areas) {
        ...
        if (iITance(area, _chart_area_group)) if (area.getFrameName() == frameName) area.drawGrid(context);
    }
    for (var n in this._areas) {
        var area = this._areas[n];
        if (iITance(area, _chart_area_group) == false) if (area.getFrameName() == frameName) this.drawAreaOverlay(context, area);
    }
};
...
_chart_man.prototype.layout = function(context, frameName, left, top, right, bottom) {
...
_chart_man.prototype.onToolMouseMove = function(frameName, x, y) {
    ...
    for (var n in this._dataSources) {
        if (n.indexOf(frameName) == 0) {
            ...
            if (iITance(ds, _main_data_source)) if (ds.toolManager.acceptMouseMoveEvent(x, y)) ret = true;
        }
    }
    return ret;
};
...
_chart_man.prototype.onToolMouseDown = function(frameName, x, y) {
    ...
    for (var n in this._dataSources) {
        if (n.indexOf(frameName) == 0) {
            var ds = this._dataSources[n];
            if (iITance(ds, _main_data_source)) if (ds.toolManager.acceptMouseDownEvent(x, y)) ret = true;
        }
    }
    return ret;
};
...
_chart_man.prototype.onToolMouseUp = function(frameName, x, y) {
    ...
    for (var n in this._dataSources) {
        if (n.indexOf(frameName) == 0) {
            var ds = this._dataSources[n];
            if (iITance(ds, _main_data_source)) if (ds.toolManager.acceptMouseUpEvent(x, y)) ret = true;
        }
    }
    return ret;
};
...
_chart_man.prototype.onToolMouseDrag = function(frameName, x, y) {
    ...
    for (var n in this._dataSources) {
        if (n.indexOf(frameName) == 0) {
            var ds = this._dataSources[n];
            if (iITance(ds, _main_data_source)) if (ds.toolManager.acceptMouseDownMoveEvent(x, y)) ret = true;
        }
    }
    return ret;
};
...
_chart_man.prototype.onMouseUp = function(frameName, x, y) {
    ...
    if (this._capturingMouseArea) {
        if (this._capturingMouseArea.onMouseUp(x, y) == null && this._dragStarted == false) {
            if (this._selectedFrame != null && this._selectedFrame != frame) this._selectedFrame.select(null);
            if (this._capturingMouseArea.isSelected()) {
                if (!this._captureMouseWheelDirectly) $(this._overlayCanvas).unbind("mousewheel");
                frame.select(null);
                this._selectedFrame = null;
            } else {
                if (this._selectedFrame != frame) if (!this._captureMouseWheelDirectly) $(this._overlayCanvas).bind("mousewheel", mouseWheel);
...
_chart_man.prototype.create_indtorAndRange = function(areaName, indicName, notLoadSettings) {
    var indic, range;
    switch (indicName) {
      case "MA":
        indic = new __indtor();
    ...
    if (!notLoadSettings) indic.setParameters(ChartSettings.get().indics[indicName]);
    return {
        indic:indic,
        range:range
    };
...
_chart_man.prototype.set_indtor = function(areaName, indicName) {
    ...
    if (area == undefined || area.getNameObject().getCompAt(2) == "main") return false;
...
Timeline.prototype.move = function(x) {
    if (this.isLatestShown()) _chart_man.getInstance().getArea(this.getDataSourceName() + ".mainRange").setChanged(true);
...
Timeline.prototype.scale = function(s) {
    this._scale += s;
    if (this._scale < 0) this._scale = 0; else if (this._scale >= Timeline._ItemWidth.length) this._scale = Timeline._ItemWidth.length - 1;
    this.updateMaxItemCount();
    if (this._selectedIndex >= 0) this.validateSelectedIndex();
};
...
Range.prototype.getSelectedPosition = function() {
    ...
    if (this._selectedValue > -Number.MAX_VALUE) return this.toY(this._selectedValue);
    return -1;
};
...
Range.prototype.getSelectedValue = function() {
    ...
    if (this._selectedPosition < area.getTop() + 12 || this._selectedPosition >= area.getBottom() - 4) return -Number.MAX_VALUE;
...
Range.prototype.toY = function(value) {
    if (this._ratio > 0) return this._top + Math.floor((this._maxValue - value) * this._ratio + .5);
    return this._top;
};
...
Range.prototype.setRange = function(minValue, maxValue) {
    var mgr = _chart_man.getInstance();
    var area = mgr.getArea(this.getAreaName());
    if (this._minValue == minValue && this._maxValue == maxValue && !area.isChanged()) return;
    this._updated = true;
    this._minValue = minValue;
    this._maxValue = maxValue;
    this._gradations = [];
    ...
    if (h << 2 < 24) {
        ...
        do {
            if (h < 8) {
                ...
                if (v == 0 || v == 20 || v == 50 || v == 80 || v == 100) this._gradations.push(v);
            }
            v += interval;
        } while (v < this.getMaxValue());
    }
...
_i_data_prov.prototype.getMinMaxAt = function(index, minmax) {
    ...
    var i, cnt = this.__indtor.getOutputCount();
    for (i = 0; i < cnt; i++) {
...
DarkTheme.prototype._cTruct = function() {
    ...
    this._clors[Theme.Co.Unchanged] = "#242e47";
    this._clors[Theme.Co.Background] = "#242e47";
...
LightTheme.prototype._cTruct = function() {
    this._clors = [];
    this._clors[Theme.Co.Positive] = "#53b37b";
    this._clors[Theme.Co.Negative] = "#db5542";
    this._clors[Theme.Co.PositiveDark] = "#66d293";
    this._clors[Theme.Co.NegativeDark] = "#ffadaa";
    this._clors[Theme.Co.Unchanged] = "#fff";
    this._clors[Theme.Co.Background] = "#fff";
    this._clors[Theme.Co.Cursor] = "#aaa";
    this._clors[Theme.Co.RangeMark] = "#f27935";
    this._clors[Theme.Co._indtor0] = "#2fd2b2";
    this._clors[Theme.Co._indtor1] = "#ffb400";
    this._clors[Theme.Co._indtor2] = "#e849b9";
    this._clors[Theme.Co._indtor3] = "#1478c8";
    this._clors[Theme.Co.Grid0] = "#eee";
    this._clors[Theme.Co.Grid1] = "#afb1b3";
    this._clors[Theme.Co.Grid2] = "#ccc";
    this._clors[Theme.Co.Grid3] = "#bbb";
    this._clors[Theme.Co.Grid4] = "#aaa";
    this._clors[Theme.Co.TextPositive] = "#53b37b";
    this._clors[Theme.Co.TextNegative] = "#db5542";
    this._clors[Theme.Co.Text0] = "#ccc";
    ...
    this._clors[Theme.Co.Text3] = "#666";
    this._clors[Theme.Co.Text4] = "#444";
    this._clors[Theme.Co.LineColorNormal] = "#8c8c8c";
    this._clors[Theme.Co.LineColorSelected] = "#393c40";
    this._clors[Theme.Co.CircleColorFill] = "#ffffff";
    this._clors[Theme.Co.CircleColorStroke] = "#393c40";
...
Template.createTableComps = function(dsName) {
    ...
    if (Template.displayVolume) Template.create_indtorChartComps(dsName, "VOLUME");
...
Plotter.drawDashedLine = function(context, x1, y1, x2, y2, dashLen, dashSolid) {
    ...
    if (dY == 0) {
        ...
        var count = Math.sqrt(dX * dX + dY * dY) / dashLen + .5 << 0;
        dX = dX / count;
        dY = dY / count;
        ...
        for (var i = 0; i < count; i++) {
            ...
            context.lineTo(x1 + .5 + dashX, y1 + .5 + dashY);
            x1 += dX;
            y1 += dY;
        }
        context.stroke();
...
Plotter.createPolygon = function(context, points) {
    ...
    var i, cnt = points.length;
    for (i = 1; i < cnt; i++) context.lineTo(points[i].x + .5, points[i].y + .5);
    context.closePath();
};
...
CGridPlotter.prototype.Draw = function(context) {
    ...
    // 技术指标表格横线
    for (var n in gradations) Plotter.createHorzDashedLine(context, area.getLeft(), area.getRight(), range.toY(gradations[n]), dashLen, dashSolid);
    context.fill();
...
MainInfoPlotter.prototype.Draw = function(context) {
    ...
    // “开高低收” 字符串
    if (!Plotter.drawString(context, lan[lang].others.data+": " + year + "-" + month + "-" + date + "  " + hour + ":" + minute, rect)) return;
    if (!Plotter.drawString(context, "  "+lan[lang].others.o+": " + data.open.toFixed(digits), rect)) return;
    if (!Plotter.drawString(context, "  "+lan[lang].others.h+": " + data.high.toFixed(digits), rect)) return;
    if (!Plotter.drawString(context, "  "+lan[lang].others.l+": " + data.low.toFixed(digits), rect)) return;
    if (!Plotter.drawString(context, "  "+lan[lang].others.c+": " + data.close.toFixed(digits), rect)) return;
    if (selIndex > 0) {
        if (!Plotter.drawString(context, lan[lang].others.cf+"  : ", rect)) return;
    ...
    // 振幅数据
    if (!Plotter.drawString(context, lan[lang].others.af+"  : " + amplitude.toFixed(2) + " %", rect)) return;
    if (!Plotter.drawString(context, lan[lang].others.v+"  : " + data.volume.toFixed(2), rect)) return;
...
// Volume 矩形绘制（第二个）
_indtorPlotter.prototype.drawVolumeStick = function(context, theme, ds, first, last, startX, cW, iW, range) {
    var dark = iITance(theme, DarkTheme);
    var left = startX;
...
// MACD 矩形绘制
_indtorPlotter.prototype.drawMACDStick = function(context, theme, output, first, last, startX, cW, iW, range) {
    var left = startX;
...
// 画圆圈，暂时没有使用。
_indtorPlotter.prototype.drawSARPoint = function(context, theme, output, first, last, startX, cW, iW, range) {
    var r = iW >> 1;
    if (r < .5) r = .5;
    if (r > 4) r = 4;
    var center = startX;
...
TimelinePlotter.prototype.Draw = function(context) {
    ...
    for (var i = first; i < last; i++) {
        if (!ds.getDataAt(i)) return false
        ...
        if (n < cnt) {
            ...
            if (localDate % m == 0) {
                text = lan[lang].time[month]+ "  " +  date.toString()+lan[lang].time.d; 
                context.fillStyle = theme.getColor(Theme.Co.Text4);
...
RangePlotter.prototype.getRequiredWidth = function(context, v) {
    ...
    var theme = mgr.getTheme(this.getFrameName());
    context.font = theme.getFont(Theme.Font.Default);
    return context.measureText((Math.floor(v) + .88).toString()).width + 16;
};
...
COrderGraphPlotter.prototype.DrawBackground = function(context) {
    ...
    if (this.m_mode == 0) {
        ...
        ask_gradient.addColorStop(1, this.m_pTheme.getColor(Theme.Co.PositiveDark));
        ...
        bid_gradient.addColorStop(1, this.m_pTheme.getColor(Theme.Co.NegativeDark));
...
COrderGraphPlotter.prototype.DrawLine = function(context) {
    if (this.m_mode == 0 || this.m_mode == 1) {
        context.strokeStyle = this.m_pTheme.getColor(Theme.Co.PositiveDark);
        context.beginPath();
        context.moveTo(Math.floor(this.m_ask_points[0].x) + .5, Math.floor(this.m_ask_points[0].y) + .5);
        for (var i = 1; i < this.m_ask_points.length; i++) {
            context.lineTo(Math.floor(this.m_ask_points[i].x) + .5, Math.floor(this.m_ask_points[i].y) + .5);
        }
        context.stroke();
    }
    if (this.m_mode == 0 || this.m_mode == 2) {
        context.strokeStyle = this.m_pTheme.getColor(Theme.Co.NegativeDark);
        context.beginPath();
        context.moveTo(this.m_bid_points[0].x + .5, this.m_bid_points[0].y + .5);
        for (var i = 1; i < this.m_bid_points.length; i++) {
            context.lineTo(this.m_bid_points[i].x + .5, this.m_bid_points[i].y + .5);
        }
        context.stroke();
    }
};
...
COrderGraphPlotter.prototype.FillBlack = function(context) {
    ...
    context.fillStyle = this.m_pTheme.getColor(Theme.Co.Background);
    context.beginPath();
    context.moveTo(Math.floor(ask_point[0].x) + .5, Math.floor(ask_point[0].y) + .5);
    for (var i = 1; i < ask_point.length; i++) {
        context.lineTo(Math.floor(ask_point[i].x) + .5, Math.floor(ask_point[i].y) + .5);
    }
    context.fill();
    context.beginPath();
    context.moveTo(Math.floor(bid_point[0].x) + .5, Math.floor(bid_point[0].y) + .5);
    for (var i = 1; i < bid_point.length; i++) {
        context.lineTo(Math.floor(bid_point[i].x) + .5, Math.floor(bid_point[i].y) + .5);
    }
    context.fill();
    ask_point.shift();
    ask_point.pop();
...
SelectionPlotter.prototype.Draw = function(context) {
    ...
    if (pos >= 0) Plotter.drawLine(context, area.getLeft(), pos, area.getRight(), pos);
};
...
TimelineSelectionPlotter.prototype.Draw = function(context) {
    ...
    if (minute < 10) strMinute = "0" + strMinute;
    var text = "";
    text = lan[lang].time[strMonth]+ strDate + lan[lang].time.d+"  " + strHour + ":" + strMinute;
...
RangeSelectionPlotter.prototype.Draw = function(context) {
    ...
    var digits = 2;
    if (range.getNameObject().getCompAt(2) == "main") digits = mgr.getDataSource(this.getDataSourceName()).getDecimalDigits();
...
ChartSettings.init = function() {
    ...
    for (var i = 0; i < _name.length; i++) {
        var _value = _chart_man.getInstance().create_indtorAndRange("", _name[i], true);
...
CToolObject.prototype.isValidMouseXY = function(x, y) {
    ...
    if (x < areaPos.left || x > areaPos.right || y < areaPos.top || y > areaPos.bottom) return false;
    return true;
};
...
var  CBiToolObject = new_class(CToolObject);
...
var CTriToolObject = new_class(CToolObject);
...
CBandLineObject.prototype.isSelected = function(x, y) {
    if (CBandLineObject.__classId.isSelected.call(this, x, y) == true) return true;
...
CBiParallelLineObject.prototype.isSelected = function(x, y) {
    if (CTriParallelLineObject.__classId.isSelected.call(this, x, y) == true) return true;
...
CFibFansObject.prototype.isSelected = function(x, y) {
    if (CFibFansObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    var sx = this.getPoint(0).getPosXY().x;
    var sy = this.getPoint(0).getPosXY().y;
...
CFibRetraceObject.prototype.isSelected = function(x, y) {
    if (CFibRetraceObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    var sx = this.getPoint(0).getPosXY().x;
    var sy = this.getPoint(0).getPosXY().y;
...
CHoriRayLineObject.prototype.isSelected = function(x, y) {
    if (CHoriRayLineObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    var sy = this.getPoint(0).getPosXY().y;
    var sx = this.getPoint(0).getPosXY().x;
...
CHoriSegLineObject.prototype.isSelected = function(x, y) {
    if (CHoriSegLineObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    var sy = this.getPoint(0).getPosXY().y;
    var sx = this.getPoint(0).getPosXY().x;
...
CHoriStraightLineObject.prototype.isSelected = function(x, y) {
    if (CHoriStraightLineObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    var sy = this.getPoint(0).getPosXY().y;
    if (y > sy + 4 || y < sy - 4) return false;
...
CRayLineObject.prototype.isSelected = function(x, y) {
    if (CRayLineObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    var sx = this.getPoint(0).getPosXY().x;
    var ex = this.getPoint(1).getPosXY().x;
...
CSegLineObject.prototype.isSelected = function(x, y) {
    if (CSegLineObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    if (this.isWithRect(this.getPoint(0), this.getPoint(1), c) == false) return false;
    if (this.calcDistance(this.getPoint(0), this.getPoint(1), c) < 4) {
        this.select();
        return true;
    }
...
CStraightLineObject.prototype.isSelected = function(x, y) {
    if (CStraightLineObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    if (this.calcDistance(this.getPoint(0), this.getPoint(1), c) < 4) {
        this.select();
...
CTriParallelLineObject.prototype.isSelected = function(x, y) {
    ...
    if (this.calcGap(r1, x, y) > 4 && this.calcGap(r2, x, y) > 4 && this.calcGap(r3, x, y) > 4) return false;
    this.select();
    return true;
};
...
CVertiStraightLineObject.prototype.isSelected = function(x, y) {
    if (CVertiStraightLineObject.__classId.isSelected.call(this, x, y) == true) return true;
    var c = new CPoint("frame0.k0");
    c.setPosXY(x, y);
    var sx = this.getPoint(0).getPosXY().x;
...
CToolManager.prototype.acceptMouseMoveEvent = function(x, y) {
    if (this.selectedObject == -1) {
        ...
        if (curr != null && curr.getState() != CToolObject.state.AfterDraw) return curr.acceptMouseMoveEvent(x, y);
    } else {
        var sel = this.toolObjects[this.selectedObject];
        if (sel.getState() == CToolObject.state.Draw) return sel.acceptMouseMoveEvent(x, y);
        sel.unselect();
        this.selectedObject = -1;
    }
...
CToolManager.prototype.acceptMouseDownEvent = function(x, y) {
    ...
    if (this.selectedObject == -1) {
        var curr = this.toolObjects[this.getToolObjectCount() - 1];
        if (curr != null && curr.getState() != CToolObject.state.AfterDraw) return curr.acceptMouseDownEvent(x, y);
    } else {
        var sel = this.toolObjects[this.selectedObject];
        if (sel.getState() != CToolObject.state.BeforeDraw) return sel.acceptMouseDownEvent(x, y);
    }
    return false;
};
...
CToolManager.prototype.acceptMouseDownMoveEvent = function(x, y) {
    ...
    if (this.selectedObject == -1) {
        var curr = this.toolObjects[this.getToolObjectCount() - 1];
        if (curr != null && curr.getState() == CToolObject.state.Draw) return curr.acceptMouseDownMoveEvent(x, y);
        return false;
    } else {
        var sel = this.toolObjects[this.selectedObject];
...
CToolManager.prototype.acceptMouseUpEvent = function(x, y) {
    if (this.mouseDownMove == true) {
        if (this.selectedObject == -1) {
            var curr = this.toolObjects[this.getToolObjectCount() - 1];
            if (curr != null && curr.getState() == CToolObject.state.Draw) return curr.acceptMouseUpEvent(x, y);
            return true;
        } else {
            var sel = this.toolObjects[this.selectedObject];
            if (sel.getState() != CToolObject.state.BeforeDraw) return sel.acceptMouseUpEvent(x, y);
        }
    }
    if (this.selectedObject != -1) {
        return true;
...
DrawBiParallelRayLinesPlotter.prototype._cTruct = function(name, toolObject) {
    DrawBiParallelRayLinesPlotter.__classId._cTruct.call(this, name, toolObject);
    this.toolObject = toolObject;
    this.ctrlPtsNum = 3;
    this.ctrlPts = new Array(new Array(this.ctrlPtsNum), new Array(2));
    this.getCtrlPts();
    ...
    for (var i = 0; i < count; i++) {
        ...
        switch (state) {
          case CToolObject.state.BeforeDraw:
            toolObject.getPlotter().theme = _chart_man.getInstance().getTheme(this.getFrameName());
...
function KLineMouseEvent() {
    $(document).ready(function() {
        ...
        $("#chart_input_interface").submit(function(e) {
            ...
            switch (command) {
              case "set future list":
                _chart_man.getInstance().getChart().setFutureList(content);
                break;

              case "set current depth":
                _chart_man.getInstance().getChart().updateDepth(content);
                break;
        ...
        $("#chart_dsy li").click(function() {
            ...
            _chart_man.getInstance().getChart().setCurrentFuture(content);
        });
        $("#chart_container .chart_dropdown .chart_dt_one").mouseover(function() {
        ...
        $("#chart_tabbar li a").click(function() {
            ...
            ChartSettings.save();
            if (Template.displayVolume == false) _chart_man.getInstance().getChart().set_indtor(1, name); else _chart_man.getInstance().getChart().set_indtor(2, name);
        });
        ...
        $("#chart_select_main__indtor a").click(function() {
            ...
            ChartSettings.save();
            var mgr = _chart_man.getInstance();
            if (!mgr.setMain_indtor("frame0.k0", name)) mgr.removeMain_indtor("frame0.k0");
            mgr.redraw("All", true);
        });
...
function clear_refresh_counter() {
    window.KLINEBOJ.clearInterval(refresh_handler);
    refresh_counter = 0;
    var lang = _chart_man.getInstance().getLanguage();
    if (lang == "en") {
        $("#chart_updated_time_text").html(refresh_counter + "s");
...
function readCookie() {
    ...
    _chart_man.getInstance().getChart().setMain_indtor(tmp.charts.mIndic);
...
main = function(olan, olang) {
    lan[olang] = olan
    def.lang = olang
    window.KLINEBOJ = {}
    window.KLINEBOJ.onPushingStarted = function(callback) {
        Pushing.Start(callback);
    };
    window.KLINEBOJ.onPushingResponse = function(marketFrom, type, coinVol, data) {
        Pushing.Response(marketFrom, type, coinVol, data);
    };
    window.KLINEBOJ.onPushingStop = function() {
        Pushing.Stop();
    };
    window.KLINEBOJ._KlineIndex = function(content) {
        Template.displayVolume = false;
        refreshTemplate();
        readCookie();
        if (content == 0) _chart_man.getInstance().getChart().setKlineIndex("17"); else if (content == 3) _chart_man.getInstance().getChart().setKlineIndex("18"); else return;
    };
    window.KLINEBOJ._display_future_list = function() {
        $("#chart_dsy")[0].style.display = "block";
    };
    window.KLINEBOJ._set_current_language = function(content) {
        chart_switch_language(content);
    };
    window.KLINEBOJ._set_current_depth = function(content) {
        _chart_man.getInstance().getChart().updateDepth(content);
    };
    window.KLINEBOJ._set_current_coin = function(content) {
        _chart_man.getInstance().getChart().setCurrentCoin(content);
    };
    // window.KLINEBOJ._set_current_url = function(content) {
    //     _Global.url = content;
    // };
    window.KLINEBOJ._set_future_list = function(content) {
        _chart_man.getInstance().getChart().setFutureList(content);
    };
    window.KLINEBOJ._set_current_future = function(contractID) {
        _chart_man.getInstance().getChart().setCurrentFuture(contractID);
    };
    window.KLINEBOJ._set_init_current_future = function(contractID) {
        _chart_man.getInstance().getChart().setCurrentFutureNoRaise(contractID);
    };
    window.KLINEBOJ._set_current_contract_unit = function(str) {
        _chart_man.getInstance().getChart().setCurrentContractUnit(str);
    };
    window.KLINEBOJ._set_money_type = function(str) {
        _chart_man.getInstance().getChart().setCurrentMoneyType(str);
    };
    window.KLINEBOJ._set_usd_cny_rate = function(rate) {
        _chart_man.getInstance().getChart()._usd_cny_rate = rate;
    };
    window.KLINEBOJ._setCaptureMouseWheelDirectly = function(b) {
        _chart_man.getInstance().setCaptureMouseWheelDirectly(b);
    };
    window.KLINEBOJ._current_future_change = new mEves();
    window.KLINEBOJ._current_theme_change = new mEves();
    /*
     window.KLINEBOJ._current_future_change.addHandler(o, func);
     window.KLINEBOJ._current_theme_change.addHandler(o, func);
     window.KLINEBOJ._current_future_change.raise(content);
     window.KLINEBOJ._current_theme_change.raise(content);
     */
    chart_switch_language(def.lang);
    KLineMouseEvent();
    console.log(document.getElementById("chart_mainCanvas"))
    _chart_man.getInstance().bindCanvas("main", document.getElementById("chart_mainCanvas"));
    _chart_man.getInstance().bindCanvas("overlay", document.getElementById("chart_overlayCanvas"));
...
function refreshTemplate() {
    _Global.chartMgr = DefaultTemplate.loadTemplate("frame0.k0", "行情图" + _Global.mark_from + _Global.time_type, "frame0.order", "0.order", "frame0.trade", "0.trade");
    _chart_man.getInstance().redraw("All", true);
}
...
function chart_switch_language(lang) {
    ...
    _chart_man.getInstance().getChart().setTitle();
}
...
function switch_theme(name) {
    ...
    $("#chart_select_theme a").removeClass("selected");
    if (name == "dark") {
        $("#chart_tSkin").find("a").each(function() {
            if ($(this).attr("name") == "dark") {
                $(this).addClass("selected");
            }
        });
    ...
    $("#chart_output_interface_text").val(JSON.stringify(a));
    $("#chart_output_interface_submit").submit();
    window.KLINEBOJ._current_theme_change.raise(name);
...
function switch_tools(name) {
    ...
    if (name == "on") {
        ...
        if (_chart_man.getInstance()._drawingTool == _chart_man.DrawingTool.Cursor) {
            $("#chart_Cursor").parent().addClass("selected");
        } else if (_chart_man.getInstance()._drawingTool == _chart_man.DrawingTool.CrossCursor) {
            $("#chart_CrossCursor").parent().addClass("selected");
        }
    } else if (name == "off") {
        ...
        _chart_man.getInstance().setRunningMode(_chart_man.getInstance()._beforeDrawingTool);
        _chart_man.getInstance().redraw("All", true);
    }
    on_size();
}
...
function switch_indic(name) {
    ...
    if (name == "on") {
        ...
        if (Template.displayVolume == false) _chart_man.getInstance().getChart().set_indtor(2, value); else _chart_man.getInstance().getChart().set_indtor(2, value);
        ...
        _chart_man.getInstance().getChart().set_indtor(2, "NONE");
        var tmp = ChartSettings.get();
...
function calcPeriodWeight(period) {
    ...
    ChartSettings.save();
    $("#chart_tph").find("li").each(function() {
...
switchPeriod = function (name) {
    switch_period(name);
}
setData = function (data) {
    _Global._KData = data
    var p = _Global.tagMapPeriod[_Global.period];
    _chart_man.getInstance().getChart().strIsLine = false;
    _chart_man.getInstance().setChartStyle("frame0.k0", ChartSettings.get().charts.chartStyle);
    _chart_man.getInstance().getChart().setCurrentPeriod(p);
    var settings = ChartSettings.get();
    settings.charts.period = _Global.period;
    ChartSettings.save();
    _Global.chartMgr.updateData("frame0.k0", _Global._KData);
    _chart_man.getInstance().redraw("All", true);
    // 分时与其他时间刻度渲染的区别
    if (isLine === "line") {
        _chart_man.getInstance().getChart().strIsLine = true;
        _chart_man.getInstance().setChartStyle("frame0.k0", "Line");
        _chart_man.getInstance().getChart().setCurrentPeriod("01m");
        var settings = ChartSettings.get();
        settings.charts.period = "line";
        ChartSettings.save();
        _chart_man.getInstance().redraw("All", true);
    }
} 
export default {main,setData,switchPeriod,_Global}
